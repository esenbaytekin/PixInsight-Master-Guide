...
!!! warning "PixInsight 1.9.3 sürüm doğrulaması"
    `SpectrophotometricColorCalibration` processinin menü konumu, section/control etiketleri, catalog seçenekleri, sensor/filter database davranışı, white reference, background, narrowband, rejection, output ve varsayılan değerleri gerçek PixInsight 1.9.3 kurulumu ve birincil kaynakla doğrulanmayı bekliyor. Bu sayfa doğrulanmamış UI adı veya parametre reçetesi üretmez.
...
| --- | --- | --- | --- |
| Process menu location | Exact menü yolu | 1.9.3 ekranı | Bekliyor |
| Section names | Exact panel/section adları | 1.9.3 ekranı | Bekliyor |
| Catalog selector | Catalog adları ve davranış | UI + documentation | Bekliyor |
| White reference controls | Seçenekler ve anlamları | UI + documentation | Bekliyor |
...
| Narrowband controls | Varlık, ad ve kapsam | UI + birincil kaynak | Bekliyor |
| Background neutralization controls | Exact ilişki ve output | UI + test | Bekliyor |
| Star rejection controls | Exact seçim/rejection davranışı | UI + log | Bekliyor |
| Output/log controls | Üretilen çıktı ve mesajlar | UI + console/log | Bekliyor |
...
| Astrometric requirement | Process-bazlı doğrulama bekliyor | Process-bazlı doğrulama bekliyor |
...
| --- | --- | --- |
| Process başlamıyor | UI, input state, log | [Sorun Giderme](spcc-troubleshooting.md) |
...
| Narrowband sonuç anlamsız | Workflow hedefi ve mode doğrulaması | [Narrowband](spcc-narrowband.md) |
