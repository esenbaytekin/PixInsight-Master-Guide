...
!!! info "Sayfa Bilgisi"
    ...
    **Anahtar kelimeler:** `çekim planlama` · `target visibility` · `weather` · `Moon phase` · `exposure time` · `calibration frames` · `dithering` · `session planning`

**Durum: Taslak**

## Amaç

Bu bölüm, Çekim Planlama konusunun PixInsight tabanlı monokrom astrofotoğraf işleme akışındaki yerini ve temel karar noktalarını açıklamak için hazırlanmıştır.
