...
!!! info "Sayfa Bilgisi"
    ...
    **Anahtar kelimeler:** `SNR` · `signal-to-noise ratio` · `sinyal` · `noise` · `gürültü` · `dynamic range` · `integration` · `clipping`

**Durum: Taslak**

## Amaç

Bu bölüm, SNR ve Dinamik Aralık konusunun PixInsight tabanlı monokrom astrofotoğraf işleme akışındaki yerini ve temel karar noktalarını açıklamak için hazırlanmıştır.
